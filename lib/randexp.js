...
module.exports = class RandExp {
   ...
   * @return {string}
   */
  gen(match, useChoices) {
    this.lastChoices = [];
    if (useChoices) {
      this.useChoices = [...useChoices];
    }
  ...
  _gen(token, groups, match) {
    ...
    switch (token.type) {
      ...
      case types.REPETITION:
        ...
        n = this._randInt(token.min,
          token.max === Infinity ? token.min + this.max : token.max);

        str = '';
        if (this.useChoices) {
          let len = this.useChoices.shift();
          for (i = 0; i < len; i++) {
   ...
   */
  _randBool() {
    return !this._randInt(0, 1);
  }

  _randInt(...args) {
    if (this.useChoices) {
      return this.useChoices.shift();
    }

    const rv = this.randInt(...args);
    this.lastChoices.push(rv);
    return rv;
  }
  ...
  _randSelect(arr, groups, match) {
    if (arr instanceof DRange) {
      if (this.useChoices) {
        return [this.useChoices.shift(), 0];
      }

      if (match) {
        const next = match.charCodeAt(0);
        if (arr.intersect(new DRange(next, next).length === 1)) {
          this.lastChoices.push(next);
          return [next, 1];
        }
      }
      return [arr.index(this._randInt(0, arr.length - 1)), 0];
    }

    if (this.useChoices) {
      return [arr[this.useChoices.shift()], 0];
    }

    if (match) {
      let best = null;
      let bestI = 0;
      let prev = [...this.lastChoices];
      arr.forEach((opt, i) => {
      ...
      this.lastChoices = prev;
      this.lastChoices.push(bestI);
      return [arr[bestI], best[1]];
    }

    return [arr[this._randInt(0, arr.length - 1)], 0];
  }
